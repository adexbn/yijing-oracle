...
├── codemagic.yaml       # Codemagic 构建 + TestFlight 发布
├── .github/
│   └── workflows/
│       └── ios.yml      # GitHub Actions 免费编译验证
└── Yijing/
...
- 已配好 `codemagic.yaml`。流程：
  ...
  1. 把 `yijing-ios` 目录作为一个仓库推到 GitHub / GitLab / Bitbucket。
...
- 已配好 `.github/workflows/ios.yml`：push 即触发，先 `xcodegen generate`，再做 **不签名的编译验证**——零 secrets 即可确认代码能否编译通过。
