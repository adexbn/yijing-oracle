...
- 最后更新：2026-09-25
- 最近提交：`2f0e3d2`（分支 `main`，已 push；更准确以 `git log` 为准）
- 待提交：无 —— `2f0e3d2` 只加了 `docs/HF_TEST_KIT.md` 一个文件，**未改任何代码**
...
8. **（新增，等用户实测）端侧模型是否换成 Qwen3.5-2B**：调研结论见 `docs/MODEL_SELECTION.md`（只值得试 Qwen3.5-2B；iOS 现役 llama.cpp `b10809` 已原生支持 `qwen35` 架构与 Metal `gated_delta_net` 内核 ⇒ **无需换引擎**）。**是否动手取决于用户实测**：若语义「对文字的理解不亚于 Qwen3-1.7B」再换。该文档第 5 节留了 4 条 `[待验证]`：用户实测语义、iOS 真机验证 XCFramework 是否真含该内核、换模型必须同改采样参数、Qwen3.5 的 `/think` 软开关与 Qwen3「空思考块」机制是否等价。
   > **2026-09-25 已备好 HF 测试包**（`docs/HF_TEST_KIT.md`，`2f0e3d2`）：用户将自行提交到 Hugging Face 实测。**模型（链接已逐个 API 实测存在）**：候选 = `Qwen/Qwen3.5-2B`（原始权重）/ `taobao-mnn/Qwen3.5-2B-MNN`（Android 侧，同官方组织）/ `lmstudio-community/Qwen3.5-2B-GGUF`（iOS 侧，**App 里写死的下载源就是它，不用换源**）；现役 = `Qwen/Qwen3-1.7B` / `taobao-mnn/Qwen3-1.7B-MNN` / `lmstudio-community/Qwen3-1.7B-GGUF`；免装环境试跑 = `webml-community/Qwen3.5-WebGPU` 与 `webml-community/qwen3-webgpu`（后者模型下拉选 1.7B）。**「之前测试的问题」** = 382 条主回归语料（valid 114 / A 60·B 16·C 8·D 30）+ 12 条真机级 V 批 + 8 条带完整卦象的现成用例（卦辞爻辞从 `HexagramDb.kt` / `YaoDbShang.kt` / `YaoDbXia.kt` 自动抽取）。**两条与直觉相反的实测结论**：① 换 2B **体积反而略小**（iOS Q4_K_M 小 11.6 MB / Android MNN 文本集合小 45 MB）；② 候选 MNN 多一个 `llm.mnn.json`，且带**本项目不需要的视觉分支** `visual.mnn*`（196 MB）。**仍未改任何代码**；需注意 Qwen3.5-2B 官方默认 non-thinking，实测必须显式 `enable_thinking=True` 或消息带 `/think`，否则测出来必然偏差（属测错）。
