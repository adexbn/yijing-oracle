...
- 最后更新：2026-09-25
- 最近提交：`9c78e05`（分支 `main`，已 push；更准确以 `git log` 为准）
- 待提交：无 —— `9c78e05` / `439f70f` / `2f0e3d2` 三个提交都只动文档，**未改任何代码**
- 文档索引：`docs/PERF_AUDIT.md`（性能审计与端侧速度调研）、`docs/IOS_LOG_CHECKLIST.md`（iOS 真机日志取证清单）、`docs/MODEL_SELECTION.md`（**模型选型调研：为什么只值得试 Qwen3.5-2B、换模型要动哪些代码、在线试用入口**）、`docs/ANDROID_PERF_OPTIONS.md`（安卓加速路线调研）、`docs/HF_TEST_KIT.md`（**HF 侧模型对比测试包：现役/候选模型清单 + 逐字 SYSTEM 提示词 + 382 条真实问句 + 采样参数 + 判分口径**，由 `build_hf_kit.py` 从仓库源码自动导出）
...
| 端 | 功能状态 | 编译验证 | 备注 |
| --- | --- | --- | --- |
| iOS | 输入有效性拦截（P3 规则闸）已接入；**已真正开启 Qwen3 思考模式**（对齐 Android 与硬约束） | ✅ 已装机可跑的是 **`version=1.2.0 build=7`**（真机实测通过）；本轮改为 **`1.3.0 / build 8`**，CI `iOS Build #28` 编译**已通过**（success），**仅剩真机复测** | 产物 `Yijing-adhoc-ipa`。**v1.2.0 真机实测**：首次解卦 **20.6s**（含首次 Metal 着色器编译 16.9s）、第二次 **2.8s**，生成 **42.8–45.8 tok/s**，是 Android 真机 18.37 tok/s 的 **2.3–2.5 倍**；旧日志实证的 5 条 bug 全部消失。本机是 Windows 无 Xcode，Swift 改动只能靠 CI —— **`iOS Build #28`（sha `1d93d4e`）已于 2026-09-25 success，1.3.0 编译通过**，剩余仅真机复测。**遗留 1 项（详见第 5 节）**：**启动预热未生效**（非思考模式已在 1.3.0 修掉） |
...
8. **（新增，等用户实测）端侧模型是否换成 Qwen3.5-2B**：调研结论见 `docs/MODEL_SELECTION.md`（只值得试 Qwen3.5-2B；iOS 现役 llama.cpp `b10809` 已原生支持 `qwen35` 架构与 Metal `gated_delta_net` 内核 ⇒ **无需换引擎**）。**是否动手取决于用户实测**：若语义「对文字的理解不亚于 Qwen3-1.7B」再换。该文档第 5 节留了 4 条 `[待验证]`：用户实测语义、iOS 真机验证 XCFramework 是否真含该内核、换模型必须同改采样参数、Qwen3.5 的 `/think` 软开关与 Qwen3「空思考块」机制是否等价。
   > **2026-09-25 已备好 HF 测试包**（`docs/HF_TEST_KIT.md`，`2f0e3d2`）：用户将自行提交到 Hugging Face 实测。**模型（链接已逐个 API 实测存在）**：候选 = `Qwen/Qwen3.5-2B`（原始权重）/ `taobao-mnn/Qwen3.5-2B-MNN`（Android 侧，同官方组织）/ `lmstudio-community/Qwen3.5-2B-GGUF`（iOS 侧，**App 里写死的下载源就是它，不用换源**）；现役 = `Qwen/Qwen3-1.7B` / `taobao-mnn/Qwen3-1.7B-MNN` / `lmstudio-community/Qwen3-1.7B-GGUF`；免装环境试跑 = **只有候选 2B 有**（`webml-community/Qwen3.5-WebGPU`，实测 `runtime.stage: RUNNING`，覆盖 0.8B / 2B / 4B）；**现役 1.7B 没有官方 Space**（`webml-community/qwen3-webgpu` 实测只挂了 0.6B，不能测现役），且**两个模型页右侧都没有 Chat 挂件**（实测 `Qwen/Qwen3.5-2B` 与 `Qwen/Qwen3-1.7B` 都只挂了第三方 `featherless-ai`，无官方 Serverless Inference；`disabled:false` / `gated:false`）。**「之前测试的问题」** = 382 条主回归语料（valid 114 / A 60·B 16·C 8·D 30）+ 12 条真机级 V 批 + 8 条带完整卦象的现成用例（卦辞爻辞从 `HexagramDb.kt` / `YaoDbShang.kt` / `YaoDbXia.kt` 自动抽取）。**两条与直觉相反的实测结论**：① 换 2B **体积反而略小**（iOS Q4_K_M 小 11.6 MB / Android MNN 文本集合小 45 MB）；② 候选 MNN 多一个 `llm.mnn.json`，且带**本项目不需要的视觉分支** `visual.mnn*`（196 MB）。**仍未改任何代码**；需注意 Qwen3.5-2B 官方默认 non-thinking，实测必须显式 `enable_thinking=True` 或消息带 `/think`，否则测出来必然偏差（属测错）。
   > **2026-09-25 更正（`9c78e05`）**：用户反馈「HuggingFace 右边没有 Chat」。已实测 HF API 查明原因 —— **这两个模型都没挂官方 Serverless Inference**（`inferenceProviderMapping` 里只有第三方 `featherless-ai`），模型页因此不渲染 Chat 挂件；**不是账号 / 网络 / 限流问题**。同时更正此前写错的一处：`webml-community/qwen3-webgpu` 实测 `models` 只有 `onnx-community/Qwen3-0.6B-ONNX`，**不含 1.7B**，不能用来试现役（1.7B 有浏览器权重 `onnx-community/Qwen3-1.7B-ONNX`，但官方没开 Space）。已同步修正 `docs/HF_TEST_KIT.md`：新增 §1.2.1、改 §2 表格与 §12 已知坑。**仍未改任何代码**。
